...
export function sendAvailabilityMessage(beliefs: BeliefBase, api:DeliverooApi, available:boolean): void {
    ...
    for (const teammateId of teammatesIds) {
        if (teammateId !== myId) {
          api.say(
            teammateId, 
            JSON.stringify({
              type: available ? "available_to_help" : "not_available_to_help",
              data: {
                  position: myPosition
              }
            })
