...
export const handleOnMessage = ({
    ...
    stopCurrentPlan,
    needHelp,
}: handleOnMessageProps) => async (id: string, name: string, msg: string, _reply: any ) => {
    if (
        !beliefs.getBelief<string[]>("teammatesIds")?.includes(id)
    ) {
        return;
    }
    const collaborating =
        beliefs.getBelief<boolean>("isCollaborating");
    ...
    if (message.type === "available_to_help" && !collaborating) {
        ...
        if (
            intentions.getCurrentIntention()?.type ===
                desireType.MOVE ||
