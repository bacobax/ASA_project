...
interface CachedServerConfigs{
    ...
    PARCELS_GENERATION_INTERVAL: number | string | undefined;
    ...
    RANDOM_AGENT_SPEED: number | string | undefined;
...
export const readFile = ( path: string ) => {
    ...
    return new Promise<string>( (res, rej) => {
        ...
        fs.readFile(path, { encoding: 'utf8' }, (err: NodeJS.ErrnoException | null, data: string) => {
            if (err) rej(err)
            else res(data)
        })
...
export const pddlDomain: Promise<string> = readFile(path.join(__dirname, '../../../domain-deliveroo.pddl'));
