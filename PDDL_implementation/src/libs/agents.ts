...
import { solver } from "./PDDL/pddlExecutor";
...
export class AgentBDI {
    ...
    public play(): void{
        const movementDuration = getConfig<number>("CLOCK")!;
    ...
    private async deliberate(): Promise<void> {
        ...
        if (!currentIntention || this.planAbortSignal) {
            ...
            for (const desire of this.desires.generateDesires(this.beliefs)) {
                const { path: plan = [], intention = null } = await solver(desire, this.beliefs) ?? {};
    ...
    private async executePlan(): Promise<void> {
        ...
        try {
            ...
            const fallback = this.intentions.getCurrentIntention()
                ? await solver(this.intentions.getCurrentIntention()!, this.beliefs)
                : null;
